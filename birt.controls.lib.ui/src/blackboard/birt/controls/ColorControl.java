...
public class ColorControl extends Composite {
	...
	private static final String NONE_CHOICE = Messages
			.getString("ColorBuilder.text.Auto"); //$NON-NLS-1$ 
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (choiceSet != null) {
					if (choiceSet.findChoiceByDisplayName(colorName) != null)
						colorName = choiceSet
								.findChoiceByDisplayName(colorName).getName();
				}
	...
	void initAccessible() {
		...
		combo.getAccessible().addAccessibleControlListener(
				new AccessibleControlAdapter() {
	...
	public void setChoiceSet(IChoiceSet choiceSet) {
		...
		String[] colors = ChoiceSetFactory
				.getDisplayNamefromChoiceSet(choiceSet);
		Arrays.sort(colors);
		combo.removeAll();
		combo.add(NONE_CHOICE);
		if (colors != null)
	...
	private String formatRGB(RGB rgb) {
		...
		return ColorUtil.format(
				ColorUtil.formRGB(rgb.red, rgb.green, rgb.blue),
	...
	public Point computeSize(int wHint, int hHint, boolean changed) {
		...
		Point labelSize = combo.computeSize(labelExtent.x, labelExtent.y,
				changed);
