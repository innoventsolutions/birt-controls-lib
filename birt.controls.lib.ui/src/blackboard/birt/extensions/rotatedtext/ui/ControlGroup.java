...
public class ControlGroup {
	...
	public void load(RotatedTextData data) {
		...
		RGB fontRGB = new RGB(data.fontColor.r, data.fontColor.g,
				data.fontColor.b);
		this.fontColor.setRGB(fontRGB);
		this.linkUrlExpression
				.setText(data.linkURL == null ? "" : data.linkURL);
	}
	...
	public void build(final Composite parent,
		...
		this.fontFamily = createFontFamilyControl(parent, "Font:",
				backgroundColor);
		{
		...
		}
		{
			...
			final DimensionControl builder = new DimensionControl(parent,
					SWT.None, RotatedTextItem.EXTENSION_NAME,
					RotatedTextItem.WRAP_POINT_PROP);
			{
				final GridData gridData = new GridData();
		...
		this.fontStyle = createFontStyleBuilder(parent, "Style:",
				backgroundColor);
		...
		if (handler != null) {
			...
			addModifyListener(this.angle, handler,
					RotatedTextItem.ROTATION_ANGLE_PROP);
			addFocusListener(this.angle, handler,
					RotatedTextItem.ROTATION_ANGLE_PROP);
			...
			addFocusListener(this.fontFamily, handler,
					RotatedTextItem.FONT_FAMILY_PROP);
	...
	private Combo createFontFamilyControl(Composite parent, String string,
	...
	private FontStyleControl createFontStyleBuilder(Composite parent,
			String string, Color backgroundColor) {
		createLabel(parent, string, backgroundColor);
	...
	private FontSizeControl createFontSizeBuilder(Composite parent,
			String string, Color backgroundColor) {
		createLabel(parent, string, backgroundColor);
	...
	private interface Updater {
		void update(RotatedTextItem textItem, ControlGroup controlGroup)
	...
	static {
		...
		UPDATERS.put(RotatedTextItem.TEXT_PROP, new Updater() {
			public void update(final RotatedTextItem textItem,
	...
	public void updateModel(final RotatedTextItem textItem,
			final String propName) throws SemanticException {
