...
{
  public static BufferedImage createRotatedTextImage( final String text, final RotatedTextData data, final int dpi, final double baseSize,
                                                      final String baseSizeUnits )
  {
    ...
    try
    {
      ...
      final BufferedImage stringImage = new BufferedImage( 1, 1, BufferedImage.TYPE_INT_ARGB );
      g2d = (Graphics2D) stringImage.getGraphics();
      g2d.setFont( font );
      final LineInfo[] lines;
      {
        double inches = DimensionUtil.convertTo( data.wrapPoint, "in", "in", baseSize, baseSizeUnits, dpi );
      ...
      g2d.dispose();
      g2d = null;
      return createRotatedImage( lines, width, height, data.angle, data.fontColor.getAwtColor() );
    }
    catch ( Exception e )
    {
      e.printStackTrace();
  ...
  private static BufferedImage createRotatedImage( final LineInfo[] lines, final int width, final int height, int angle, final Color color )
  {
    angle = angle % 360;
    ...
    if ( angle == 90 )
      return renderRotatedObject( lines, -Math.PI / 2, height, width, -width, 0, color );

    if ( angle == 180 )
      return renderRotatedObject( lines, Math.PI, width, height, -width, -height, color );

    if ( angle == 270 )
      return renderRotatedObject( lines, Math.PI / 2, height, width, 0, -height, color );
    ...
    if ( angle > 0 && angle < 90 )
    {
      ...
      return renderRotatedObject( lines, angleInRadians, dW, dH, -width * sineTheta * sineTheta, width * sineTheta * cosTheta, color );
    }
    ...
    if ( angle > 90 && angle < 180 )
    {
      ...
      return renderRotatedObject( lines, angleInRadians, dW, dH, -( width + height * sineTheta * cosTheta ), -height / 2, color );
    }
    ...
    if ( angle > 180 && angle < 270 )
    {
      ...
      return renderRotatedObject( lines, angleInRadians, dW, dH, -( width * cosTheta * cosTheta ), -( height + width * cosTheta * sineTheta ), color );
    }
    ...
    if ( angle > 270 && angle < 360 )
    {
      ...
      return renderRotatedObject( lines, angleInRadians, dW, dH, ( height * cosTheta * sineTheta ), -( height * sineTheta * sineTheta ), color );
    }

    return renderRotatedObject( lines, 0, width, height, 0, 0, color );
  }

  private static BufferedImage renderRotatedObject( final LineInfo[] lines, final double angle, final int width, final int height, final double tx,
                                                    final double ty, final Color color )
  {
    ...
    {
      lineInfo.textLayout.draw( g2d, 0, y + (int) Math.ceil( lineInfo.textLayout.getLeading() + lineInfo.textLayout.getAscent() ) );
  ...
  public static LineInfo[] wrapText( final Graphics2D g2d, final String text, final int wrapPoint )
  {
  ...
  private static final class WrapInfo
  {
    ...
    public WrapInfo( String segment, String remainder, Rectangle2D bounds, boolean hasMore )
    {
  ...
  public static WrapInfo getWrapInfo( final Graphics2D g2d, final String line, final int wrapPoint )
  {
